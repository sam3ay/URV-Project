...
def build_metadata(identifier, path, list_of_paths, input_dict={}):
    ...
    try:
        experiment = next(gen_dict_extract(identifier, input_dict[exp_name]), None)
    except KeyError:
        input_dict[exp_name] = xmldictconv(exp_name)
        experiment = next(gen_dict_extract(identifier, input_dict[exp_name]), None)
    return (input_dict, dictquery(experiment, list_of_paths))